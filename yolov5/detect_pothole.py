        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    fontFace = font, fontScale = 0.6, color=(255,255,255), thickness=2, lineType = cv2.LINE_AA)
                    
        new_width = 1080  # 원하는 가로 크기
        new_height = 720  # 원하는 세로 크기
        resized_im0 = cv2.resize(im0, (new_width, new_height))

        cv2.imshow("IMAGE", resized_im0)
